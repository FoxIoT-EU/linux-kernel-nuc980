/*
 *  linux/drivers/net/can/nuc980_can.c
 *
 *  NUC980 CAN driver
 *
 *
					...
					struct net_device *dev)
{
	...
	can_put_echo_skb(skb, dev, msg_obj_no - C_CAN_MSG_OBJ_TX_FIRST, 0);
...
static void c_can_do_tx(struct net_device *dev)
{
	...
	for (/* nix */; (priv->tx_next - priv->tx_echo) > 0; priv->tx_echo++) {
		...
		if (!(val & (1 << (msg_obj_no - 1)))) {
			// val is unused, avoid warning
			val = can_get_echo_skb(dev,
					msg_obj_no - C_CAN_MSG_OBJ_TX_FIRST, NULL);
...
static struct net_device *alloc_c_can_dev(void)
{
	struct net_device *dev;
	...
	netif_napi_add_weight(dev, &priv->napi, c_can_poll, C_CAN_NAPI_WEIGHT);
...
static void free_c_can_dev(struct net_device *dev)
{
	free_candev(dev);
}
...
static int register_c_can_dev(struct net_device *dev)
{
	struct c_can_priv *priv = netdev_priv(dev);
	int err;
...
static int c_can_plat_probe(struct platform_device *pdev)
{
	...
	if (!res.start /*|| resource_size(&res) < NUC980_CAN_REG_SIZE*/) {
		dev_err(dev, "mem region out of range\n");
		ret = -EINVAL;
		goto free_ndev;
	}

	dev_set_name(dev, "%08x.nuc980-can", (unsigned int)res.start);
	...
	ret = register_c_can_dev(ndev);
	if (ret) {
		dev_err(dev, "unable to register can device\n");
		goto disable_clk;
	}
...
static const struct of_device_id nuc980_can_dt_ids[] = {
	{ .compatible = "nuvoton,nuc980-can" },
	{},
};
MODULE_DEVICE_TABLE(of, nuc980_can_dt_ids);

static struct platform_driver nuc980_can_driver = {
	.probe = c_can_plat_probe,
	.driver = {
		.name		= "nuc980-can",
		.of_match_table	= nuc980_can_dt_ids,
	},
};

static int __init nuc980_can_init(void)
{
	return platform_driver_register(&nuc980_can_driver);
}
device_initcall(nuc980_can_init);
